...
volatile int stopindex = -1; //nem állítja meg az adatgyujtést

ISR(ANALOG_COMP_vect)
{
  DLOG("Comparator Interrupt");
  //setup COMPARATOR
  stopindex = (buff_pos + BUFFER_SIZE - 100) % BUFFER_SIZE;//az utolsó mérés indexét
  bitClear(ACSR, ACIE); //comparator interrupt leáll
}
...
ISR(ADC_vect) //a ADC conversion happened
{
  ...
  if(buff_pos == stopindex) bitClear(ADCSRA, ADIE); //disable ADC interrupts
  else buff[buff_pos] = ADCH;
...
bitSet(ADCSRA, ADIE); //no ADC Interrupt Enable
...
void loop()
{
  if(buff_pos == stopindex)
  {
    ...
    Serial.write((unsigned char *)&pakhead, sizeof(sserialpacketheader));

    int packet_start = stopindex + packet_index * packet_buffer_size;
    if(packet_start + packet_buffer_size < BUFFER_SIZE)
    {
      Serial.write((unsigned char *)&(buff[packet_start]), packet_buffer_size);
    }else{
      Serial.write((unsigned char *)&buff[packet_start], BUFFER_SIZE - packet_start);
      Serial.write((unsigned char *)&buff[0], packet_start + packet_buffer_size - BUFFER_SIZE);
    }
    ...
    if(++packet_index < max_packet_per_msg)
    {
      ...
      packet_index = 0;
      buff_pos = 0; stopindex = -1;

      delay(500); //a small pause very needed, to recieve the correct number of bytes
