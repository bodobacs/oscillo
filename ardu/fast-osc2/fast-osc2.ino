...
void loop()
{
  ...
  {
    //visual feedback
    digitalWrite(13, blink = ~blink);   // turn the LED on (HIGH is the voltage level)

    volatile unsigned int ret = BUFFER_SIZE;
    volatile unsigned char *pnext = buff;

    while(pnext != buff + BUFFER_SIZE){
      ret = Serial.write((unsigned char *)(pnext), (buff+BUFFER_SIZE)-pnext);
      pnext += ret;
      Serial.println("--");
    }
